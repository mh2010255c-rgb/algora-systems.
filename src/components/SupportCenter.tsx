    ...
    <div className="space-y-12">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-sm md:text-base text-slate-600 max-w-xl mx-auto leading-relaxed">
          نحن في <span className="text-blue-600 font-bold">Algora Systems</span> فخورون بتقديم دعم فني حقيقي، فوري ومحلي بالكامل داخل الجزائر لحل أي انشغالات أو تدريب العمال والمسيرين.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-white/80 border border-slate-850 rounded-2xl shadow-xl flex flex-col h-[560px] overflow-hidden">
          
          {/* Chat Header */}
          <div className="bg-slate-50/60 px-5 py-3.5 border-b border-slate-850 flex items-center justify-between text-right">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center border border-blue-500/20 text-slate-900 relative">
                <Bot className="w-5 h-5" />
                <span className="w-2.5 h-2.5 rounded-full bg-orange-500 absolute bottom-[-1px] left-[-1px] border-2 border-slate-950 animate-pulse"></span>
              </div>
              <div>
                <h4 className="font-extrabold text-sm text-slate-900">المساعد التقني الذكي لـ Algora</h4>
                <p className="text-[10px] text-slate-600 flex items-center gap-1">
                  <span>إجابات فورية ذكية متوفرة 24 ساعة على مدار الأسبوع</span>
                </p>
              </div>
            </div>
            <div className="text-[10px] bg-orange-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full text-emerald-400 font-bold">
              نشط الآن دج
            </div>
          </div>

          {/* Chat Messages Stage */}
          <div className="flex-1 p-5 overflow-y-auto space-y-4 text-right bg-white/90">
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  msg.role === "user" ? "bg-blue-600/30 text-blue-500" : "bg-blue-600 text-slate-900"
                }`}>
            ...
            {isSending && (
              <div className="flex gap-3 items-start ml-auto max-w-[80%]">
                <div className="w-8 h-8 bg-blue-600 text-slate-900 rounded-lg flex items-center justify-center shrink-0">
          ...
          <div className="px-5 py-2.5 bg-slate-100/30 border-t border-slate-200/60 overflow-x-auto whitespace-nowrap scrollbar-none flex gap-2 text-right">
            {suggestedQuestions.map((q, idx) => (
              <button
                key={idx}
                onClick={() => handleSendMessage(q)}
                disabled={isSending}
                className="text-[10px] md:text-xs font-bold bg-slate-100 hover:bg-blue-900/20 border border-slate-200 hover:border-blue-500/30 px-3 py-1.5 rounded-full text-slate-700 transition-all shrink-0 cursor-pointer"
              >
                {q}
              </button>
            ))}
          ...
          {/* Input Chat Send bar */}
          <div className="p-4 bg-slate-100/80 border-t border-slate-850 flex gap-2">
            <input
              type="text"
              ...
              disabled={isSending}
              className="grow text-xs md:text-sm px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:border-blue-500 text-right"
            />
            <button
              ...
              className={`p-3 rounded-xl transition-all font-bold text-xs flex items-center justify-center ${
                inputMessage.trim() && !isSending
                  ? "bg-blue-600 hover:bg-blue-500 text-slate-900 shadow-md cursor-pointer"
        ...
        <div className="lg:col-span-5 space-y-6 text-right">
          ...
          <div className="bg-white/80 border border-slate-850 p-6 rounded-2xl shadow-xl space-y-4">
            <div className="flex items-center gap-2.5 border-b border-slate-850 pb-3">
              <MessageSquareCode className="w-5 h-5 text-blue-600" />
              <div>
                <h4 className="font-extrabold text-sm text-slate-800">إرسال تذكرة دعم فني فوري</h4>
                <p className="text-[10px] text-slate-600">للحصول على معاودة اتصال من مهندسي الدعم وحل المشاكل المعقدة.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleTicketSubmit} className="space-y-3.5">
              ...
              <AnimatePresence mode="wait">
                {ticketStatus?.success && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -5 }}
                    className="p-3.5 bg-orange-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 text-xs flex gap-2 items-start"
                  >
              ...
              <div>
                <label className="block text-xs text-slate-600 font-bold mb-1">رقم الواتساب (WhatsApp)</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full text-xs pr-10 pl-3 py-2.5 bg-slate-100 border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:border-blue-500 font-mono text-right"
                    required
                  />
              ...
              <div>
                <label className="block text-xs text-slate-600 font-bold mb-1">اكتب استفسارك أو مشكلتك هنا</label>
                <textarea
                  rows={6}
                  value={ticketMessage}
                  onChange={(e) => setTicketMessage(e.target.value)}
                  placeholder="اكتب استفسارك بالتفصيل وسيقوم مهندس الدعم بالرد عليك مباشرة عبر الواتساب..."
                  className="w-full text-xs px-3 py-2.5 bg-slate-100 border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:border-blue-500 text-right leading-relaxed"
                  required
                ></textarea>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-l from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
              >
                {isSubmittingTicket ? (
                  <>
          ...
          <div className="bg-white/80 border border-slate-850 p-5 rounded-2xl shadow-xl space-y-4 text-xs leading-relaxed text-slate-700">
            <h4 className="font-bold text-slate-800 border-b border-slate-850 pb-2">تفاصيل الاتصال والتدريب الفوري 🇩🇿</h4>
            
            <div className="space-y-3">
              <div className="flex gap-3 items-start justify-end">
                <div className="text-right">
                  <h5 className="font-bold text-slate-800">الهاتف والواتساب المباشر:</h5>
                  <p className="font-mono text-blue-600 font-bold text-[13px] mt-0.5 inline-block" dir="ltr">0671037202 / +213 671 03 72 02</p>
                  <div className="mt-2.5">
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-slate-900 rounded-xl text-[10px] font-black shadow transition-all cursor-pointer"
                    >
              ...
              <div className="flex gap-3 items-start justify-end">
                <div>
                  <h5 className="font-bold text-slate-800">البريد الإلكتروني الرسمي:</h5>
                  <p className="font-mono text-slate-600 text-[11px] mt-0.5">contact@algora.dz</p>
                </div>
                <div className="p-2 bg-blue-500/10 rounded-lg text-blue-600 shrink-0">
              ...
              <div className="flex gap-3 items-start justify-end">
                <div>
                  <h5 className="font-bold text-slate-800">أوقات العمل والتواجد:</h5>
                  <p className="text-slate-600 mt-0.5">على مدار الساعة 24/7 للرد الهاتفي الفوري.</p>
                </div>
                <div className="p-2 bg-blue-500/10 rounded-lg text-blue-600 shrink-0">
              ...
              <div className="bg-blue-950/25 border border-blue-500/20 p-3.5 rounded-xl space-y-1.5 text-right">
                <h5 className="font-bold text-blue-500 text-xs flex items-center justify-end gap-1.5">
                  <PlayCircle className="w-4 h-4 shrink-0" />
                  برنامج التدريب بالفيديو وعن بعد:
                </h5>
                <p className="text-[11px] text-slate-600 leading-relaxed">
                  نحن لا نبيع البرنامج ونرحل! نوفر جلسة تواصل كاملة عبر الـ <span className="font-bold text-slate-700">AnyDesk</span> لتثبيت البرنامج، إعداد طابعتك، وتعليم عمالك كيفية استخدام المبيعات والمخزن في أقل من 10 دقائق كاملة.
                </p>
              </div>
